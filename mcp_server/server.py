...
async def call_tool(name: str, arguments: Any) -> List[TextContent]:
    ...
    try:
        ...
        if name == "get_accounts":
            ...
            transactions = transactions_data.get("allTransactions", {}).get(
                "results", []
            )
            ...
            if search:
                ...
                transactions = [
                    ...
                    if search_lower in t.get("merchant", {}).get("name", "").lower()
                ...
                result.append(
                    {
                        ...
                        "percentage": (
                            (b.get("spent", 0) / b.get("amount", 1)) * 100
                            if b.get("amount")
                            else 0
                        ),
                    }
            ...
            return [TextContent(type="text", text=json.dumps(cashflow_data, indent=2))]
            ...
            for t in transactions:
                if (
                    query_lower in t.get("merchant", {}).get("name", "").lower()
                    or query_lower in (t.get("category", {}).get("name") or "").lower()
                    or query_lower in (t.get("notes") or "").lower()
                ):
                    results.append(
                        {
                            "date": t.get("date"),
