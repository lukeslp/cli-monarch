...
class MonarchMoney(object):
    def __init__(
        ...
        self._headers = {
            ...
            "User-Agent": "cli-monarch (https://github.com/lukeslp/cli-monarch)",
        }
        if token:
        ...
        # Initialize secure session manager if using encrypted storage
    ...
    async def login(
        ...
        if save_session:
            if self._use_secure_storage and self._session_manager:
                self._session_manager.save_session({"token": self._token})
                logger.info("Session saved with encryption")
            else:
                self.save_session(self._session_file)
    ...
    def save_session(self, filename: Optional[str] = None) -> None:
        ...
        Uses encrypted storage by default for security. Can fall back to pickle
        if use_secure_storage=False in __init__.
        """
        ...
        if self._use_secure_storage and self._session_manager:
            ...
            # Legacy pickle storage (not recommended)
            if filename is None:
    ...
    def load_session(self, filename: Optional[str] = None) -> None:
        """
        Loads pre-existing auth token from encrypted storage or pickle file.

        Tries encrypted storage first (if available), then falls back to pickle.
        """
        if self._use_secure_storage and self._session_manager:
            ...
            try:
                ...
                print(f"Warning: Could not load encrypted session: {e}")
                # Fall through to try pickle file
        ...
        if os.path.exists(filename):
            ...
            raise FileNotFoundError(f"No session file found at {filename}")

    def delete_session(self, filename: Optional[str] = None) -> None:
        """
        Deletes the session file (encrypted or pickle).
        """
        if self._use_secure_storage and self._session_manager:
            self._session_manager.delete_session()

        # Also try to delete legacy pickle file
        if filename is None:
            filename = self._session_file
